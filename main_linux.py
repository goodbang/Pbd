...
def readSheet(url):
    ...
    try:
        ...
        # val_list.append(str(tval_list2[8].text).encode('utf-8'))
    ...
    try:
        for i, item in enumerate(tval_list[:14]):
            # val_list.append(str(item.text).encode('utf-8'))
            val_list.append(item.text)
        if(len(val_list) < 15):
            val_list = np.hstack((val_list, (["emp"] * (15-len(val_list)))))
        return val_list
    except IndexError as e:
...
def getDataFrame(crp_cd):
    ...
    # 한글 쿼리문 검색 문제
    try:
        df1 = df1[df1.rpt_nm.str.contains(u"유상증자")]
    ...
    try:
        ...
        error_msg_list.append("{} : {}".format(crp_cd, e))
        return pd.DataFrame([])
...
cnt = 0
try:
    for crp_cd in kospi_list[:]:
        print("kospi : {}".format(cnt))
        cnt += 1
    ...
    result.to_csv("./data/result_kospi2.csv", encoding="utf-8")

    result_list = []
    cnt = 0
    for crp_cd in kosdak_list[:]:
        print("kosdak : {}".format(cnt))
        cnt += 1
        if crp_cd in success_list:
            continue
        if crp_cd in error_list:
    ...
    print("except occured in main code")
finally:
    result.to_csv("./data/result_kosdak.csv", encoding="utf-8")
    with open("zero_list.pickle", "wb") as f:
        pickle.dump(zero_list, f)
